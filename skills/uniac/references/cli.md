...
| `uniac plan [--json] [--full] [resource]` | no | no | Resolve the target and preview the deployable. The offline verification loop. |
| `uniac link [name\|slug]` | yes | no | Bind this directory to a remote project; writes `.uniac/deploy.json`. |
| `uniac deploy [resource]` | yes | **yes** | Resolve, materialize (pull or build each image), push, register, then watch the platform until the deployment settles. |
...
`image:` is pulled locally with **your own** docker credentials
(`config.json` and its helpers, exactly like `docker pull`); a service
declaring `build:` is built locally from your project tree. Either way the
resulting image is pushed to the project registry, and the two paths are
identical from there on. A private base image your local docker cannot pull
fails the build phase.
...
phase's exit status is 6.

`uniac plan` never builds and never contacts the Docker daemon, even for a
`build:` service. It only checks that the build root, context, and Dockerfile
exist on disk.
...
it came from (after `stateful` when both apply, absent when the names match),
then an indented `start:` sub-row when it declares a start command and an
`env:` sub-row listing its variables one per line, sorted by name. Plain, then
`--full`, on one manifest:
...
never the definition it was drawn from, so the definition name is recoverable
from `uniac.yaml` and from no field of `plan --json`.

Both `plan` and `deploy` target a **deployment**: the named resource, else the
manifest's `default`, else its only deployment. As of v0.3.12 a deployment must
instantiate exactly one service. `plan` previews a multi-service deployment
happily; `deploy` refuses it with exit 5 and `deployment "<name>" instantiates
<N> services; multi-service deployments are not yet supported`.
...
`uniac plan --json database` honors it. An *undefined* flag after the
positional is silently ignored too — `uniac plan main --nope` exits 0 rather
than 2 — so a flag that had no effect is the only signal. Always write
`uniac <cmd> [flags] [positional]`. `plan`, `deploy`, and `status` locate the
project with `--dir`; `link` uses `-C`.

**`uniac auth logout`, `auth status`, and `auth token` take no flags and ignore
every argument.** Only `auth login` parses flags, so only `auth login -h`
prints help. `uniac auth logout -h` does not print help — it logs you out,
deleting `~/.uniac/auth.json`. Do not probe these three with `-h`.
...
**Their stdout carries exactly one final frame per run once the arguments
parse — nothing else, ever.**
...
completes. The frame is what remains, in two registers: the **run record** —
what the run did — and the **output** — what it established. The output follows
a blank line and is rendered at the left margin, because it is the answer, not
a step.

**A success frame** is the root line plus its immediate stages as one-liners,
one level deep and never deeper. The stages are `plan`, `link`, `build`,
`push <service>`, `deploy <service>`, and a stage may carry a summary after its
label. Nested subtasks (the build's per-image pulls) never appear. A stage
carries a duration in parentheses only when it took a second or more; the root
...
evidence inset two further levels. **`deploy` prints no
...
When the failure is retryable the root line says so:
`✗ deploy main  push — retryable  (1.8s)`.
...
The root label gains the target only once resolution succeeds. Every failure
raised by resolution itself therefore keeps the bare `deploy` — including the
...
rejection, which is raised after resolution succeeded:
`✗ deploy main  manifest  (0ms)`. So the label is a fact about how far the run
got, never a hint about what went wrong — read the code, not the label.
...
- `lifecycle` is the serving deployment's phase, printed only when it says
  something — `active` is the quiet normal and prints nothing.
- `deploying` names the deployment task in flight, e.g.
  `running: attaching volume`. Absent when nothing is running.
- `replicas` appears only when the requested count differs from the effective
  one.
...
- `hold` rows are platform-side reasons the service is not converging.
  ...
  dangling `${{...}}` reference is one — reach both commands. The other two
  only a `deploy` can add: `release record not written: <err>` when the local
  release record could not be written, and `state unread — the platform read
  path did not answer; run \`uniac status\`` when the run finished but could
  not read that service's state back.
- Under `state unread` the service's remaining rows are missing because
  **nothing was read**, not because the platform reported empty — the section
  ...
  `endpoint` fact is being asserted either way. The warning names its own
  remedy: `uniac status` is the read that settles it.
...
the record of what it did, and whatever state it established. The one exception
is a usage error (exit 2) and `-h` (exit 0): both write only to stderr and
leave stdout empty.

### Watching a run

Live rendering is a stderr concern and never changes a byte of stdout. The
decision reads only physical facts about stderr — it never inspects who spawned
the shell, and the binary contains no agent-harness variable names at all.
First match wins:
...
Only stderr's disposition is consulted — stdout's own is not part of the
decision, and its bytes are identical in every mode.
...
`uniac status` carries it in the `error` row of its error block. In both, the
exit status is the same word. **1 is deliberately unassigned** by the
...
- **Branch on the exit status; read the code for detail.** Never match
...
- **A credential the platform *rejects* is not `auth`.** Exit 3 is a credential
  ...
  (the `project` row above, and `UNIAC_PROJECT_URL` below) — a working token
  changes neither. A rejection lands under whatever the run was doing when it
  was refused: the registration — a deploy's `deploy <service>` stage — and the
  read that decides a `uniac status` give exit 8; the `push <service>` stage
  gives exit 7; and an unlinked directory's `link` stage, whose project listing
  is the only platform call that stage makes, gives exit 70 —
  `✗ deploy main  internal`, with the 401 inset under `link`. A consumer that
  branches on exit 3 to re-authenticate never catches a revoked or wrong token,
  and loops forever on a project it cannot name.
...
- **A whole-project `uniac status` swallows its other two reads' refusals.**
  Both are additive. A per-service detail it cannot read costs that service its
  `v<N>`, `kind`, `lifecycle`, `deploying`, `replicas`, `endpoint`, `volume`
  and `hold` rows — the section keeps the name and the `status` the listing
  carried — and a refused volumes read costs every volume entity section. Both
  exit 0, with no error block and no `warning` row. This is where *an omitted
  row is an absent fact* does not hold: those rows were never read, and nothing
  in the output says so. `uniac status <service>` makes no additive read, so
  the same refusal there is exit 8.
...
that fails fast rather than wedging: stdin at EOF ends the prompt at once (the
...
```sh
uniac plan --json     # offline; passing it is necessary, not sufficient
